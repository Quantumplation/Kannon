...
errorremovecontext:
    ...
    Public Function TwipsToPixelsX(ByVal X As Integer) As Integer
        Return CInt(Microsoft.VisualBasic.Compatibility.VB6.TwipsToPixelsX(X))
    End Function

    Public Function TwipsToPixelsY(ByVal Y As Integer) As Integer
        Return CInt(Microsoft.VisualBasic.Compatibility.VB6.TwipsToPixelsY(Y))
    End Function

    Public Function TwipsToFloatRangeX(ByVal X As Integer) As Single
        Return (X - Device.X.Min) / CSng(Device.X.Max)
    End Function

    Public Function TwipsToFloatRangeY(ByVal Y As Integer) As Single
        Return (Y - Device.Y.Min) / CSng(Device.Y.Max)
    End Function
            ...
            Case WT_PACKET
                With pkt
                    ...
                    If Callbacks.Count() = 0 Then
                        'Beep
                        RaiseEvent PacketArrival(Message.LParam,
                                                 .pkCursor,
                                                 .pkX,
                                                 .pkY,
                                                 .pkZ,
                                                 ...
                                                 .pkAltitude,
                                                 .pkTwist,
                                                 .pkPitch, .pkRoll, .pkYaw, .pkSerial, .pkTime)
                    Else
                        For Each tmptab In Callbacks.Values
                            tmptab.PacketArrival(Message.LParam, .pkCursor, CInt(Microsoft.VisualBasic.Compatibility.VB6.TwipsToPixelsX(.pkX)), CInt(Microsoft.VisualBasic.Compatibility.VB6.TwipsToPixelsY(.pkY)), .pkZ, .pkButtons, .pkNormalPressure, .pkTangentPressure, .pkAzimuth, .pkAltitude, .pkTwist, .pkPitch, .pkRoll, .pkYaw, .pkSerial, .pkTime)
                        Next tmptab
                    End If
                End With
    ...
    Public Function GetExtensionData(ByRef numBytes As Short) As Object
        ...
        ReDim tmpb(numBytes)
        tmpb = System.Text.ASCIIEncoding.ASCII.GetBytes(Left(pkt.pkExtension, numBytes))
